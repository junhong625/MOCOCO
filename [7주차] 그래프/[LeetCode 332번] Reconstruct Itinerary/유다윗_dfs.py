...
# 질문1: dfs 재귀에서 route[:]가 아닌 route를 넣으면 route에 v가 계속 추가되거나 결과가 제대로 안 나오는 이유 -> 참조가 들어가기 때문!
# 질문2: result = route[:]가 안 되는 이유 -> 글로벌 위치로!
# 질문3: result.append(route)가 되는 이유 -> 인자로 넣었기 때문에
# 질문4: dfs 함수를 find-함수 밖에 넣으면 문제가 생기는 이유 -> 글로벌에 넣어야 adj 나옴 / 두 함수가 같은 레벨에 있음
